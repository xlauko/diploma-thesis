...
verification algorithm that defines how the the generated state space is
...
\marginpar{Nondeterministic choice is mainly used for scheduling of threads.}
...
given state. \marginpar{Interpreted \LLVM instructions then represent an edge
between two states.}Afterwards \DIVM interprets \LLVM bitcode from that state
...
\cite{RockaiCB17}. \marginpar{$\tau$-reduction eliminates superfluous
intermediate states and leave only those that are interesting for the
verification.} It is demanded that state space reductions preserve all
...
Transformations are done by a stand-alone tool named \LART, introduced
in~\cite{Rockai15} as \emph{\LLVM} \emph{abstraction \& refinement tool}. The main
motivation behind \LART is to provide preprocessing for \LLVM -based
verification tools, simplifying their job by reducing the problem size without
compromising soundness of verification.
...
\noindent
On the other hand, in \SymDIVINE the \code{input} call generates single multi
...
whether a given state is reachable. \marginpar{Consider a path condition:\\ $x > 0
...
generate an unreachable successor.}This may be done by a simple query on the
...
A symbolic model checking is known technique for a few decades
