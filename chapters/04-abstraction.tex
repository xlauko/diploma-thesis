...
\add{ add text }
\begin{definition}
    The set of considered concrete types, called $T_C$, is defined inductively as follows:
\begin{itemize}
    \item all scalar types are concrete types $S \subseteq T_C$,
    \item if types $t_1, \dots, t_n \in T_C$ then an aggregate type composed of
        them is also concrete type $t_1 \times \dots \times
        t_n~\in~T_C$,\sidenote{A \code{void} type corresponds to an empty product
        of types.}
    \item if $t \in T_C$ then pointer to it is also a concrete type $t^* \in T_C$.
\end{itemize}
\end{definition}
...
\begin{definition}
    ...
    \begin{itemize}
        \item $T_C$ is a set of types,
        \item $V_C$ is a set of all possible \LLVM values,
        \item $I_C \colon T_C \to \mathcal{P}(V_C)$ assigns to a type a set of values,
...
values. We require that for each concrete scalar exists an abstract scalar.
A bijection between concrete scalars and abstract scalars is defined by $\alpha \colon S
\to T_A$.
\add{ TODO alpha recursion }
\begin{definition}
A set of abstract types $T_A$ is defined inductively:
    \begin{itemize}
        \item $S \subseteq T_A$ and $\alpha(S) \subseteq T_A$,
        \item if types $t_1, \dots, t_n \in T_A$ then an aggregate type composed of
        them is also concrete type $t_1 \times \dots \times
        t_n~\in~T_A$,
        \item if $t \in T_A$ then pointer to it is also a concrete type $t^* \in
            T_A$.
    \end{itemize}
\end{definition}


Additionally to concrete domain, an abstract domain has to describe how concrete
...
\begin{definition}
    ...
    \[(T_A, V_A, I_A, O_A, \alpha, (lift_s)_{s \in S}, (lower_s)_{s \in
    S}, lift_O),\] where:
\begin{itemize}
    \item $T_A$ is a set of abstract types,
    ...
    \item $\alpha \colon S \to T_A$ is a bijection between concrete and abstract
        scalars,
    \item $lift_s \colon I_C( s ) \to I_A( \alpha( s ) )$ is a function
        for scalar value abstraction of concrete type $s \in S$,
    \item $lower_s \colon I_A( \alpha^{-1}( s ) ) \to \mathcal{P}( V_C( s ) )$ defines a
        transformation from an abstract value to a corresponding set of concrete values,
    \item $lift_O \colon O_C \to O_A$ is a transformation of
        ...
        type $t$, the abstracted instruction has to return type $\alpha(t)$. Similarly
        with operation arguments.}
\end{itemize}
    \add{ lift of operations what about alpha }
\end{definition}

\add{ rework lift and ower definition to only integer types }
...
values. The definition of $lift_s$ and $lower_s$ for integer types
...
types in the domain $Z$: ${i32}_Z \times {i32}_Z \to {i32}_Z$. The operation
semantics is then defined as:
\[
    add_{i32}(a, b) =
  \begin{cases}
...
Additionally, we define memory operations \code{load},
\code{store} and \code{alloca}. Thier prototypes can be found in appendix.
\add{ describe store, load and alloca to the appendix}
...
Control flow graph of the example program. The gray
...
\begin{example}\label{ex:vpa}
    ...
    compute the reachable set of instructions using DFS from root the
    \code{x}~through all its uses \cite{LLVM:langref}. We investigate all
    instructions where \code{x}~appears on the right hand side (marked green on
    the picture below).
...
\noindent
Since the analysis found a \code{store} to variable \code{y} (in instruction the
\code{y = x + 4}), \code{y} is added to set of roots of the function
...
Symbolic variables are created as raw inputs to the program. We mark them with a
